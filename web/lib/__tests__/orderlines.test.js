const oline = require('../orderlines')
...
describe('Validate orderlines', () => {
  ...
  test('Should validate ordelines', async () => {
    ...
    const isValid = oline.isOlineValid(lines)
    expect(isValid).toBeTrue()

    const orderlines = await oline.buildOline(lines)
  ...
  test('should fail due to unknown book', async () => {
    ...
    await expect(oline.buildOline(lines)).rejects.toThrow(`Bogen med nummer 995 findes ikke`)
